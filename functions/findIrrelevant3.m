...
for iterNum=1:totalNum
    ...
    temp=reproGMR(s, model, SigmaGMR, MuGMR,leadFrames); %all frames   
    r(4).Data(:,:)=temp(id).Data(1:2,:);

    temp=reproGMR(s, model, SigmaGMR, MuGMR,leadFrames(indx),indx);
    r(3).Data(:,:)=temp(id).Data(1:2,:);
    r(3).Sigma(:,:,:)=temp(id).Sigma(1:2,1:2,:);
    ...
    if (count == period)
        ...
        temp=reproGMR(s, model, SigmaGMR, MuGMR,leadFrames(indx));
        r(2).Data(:,:)=temp(id).Data(1:2,:);
...
%         width_real=length_real*widthim/lengthim;
        ...
        tempx=-(-cntr_im(1)+(r(1).Data(2,:)-cntr_real(2)).*(widthim)./width_real);
        tempy=-(-cntr_im(2)+(r(1).Data(1,:)-cntr_real(1)).*(lengthim)./length_real);
        plot(tempx, tempy,'-','linewidth',1.5,'color','green');%groundtruth
        ...
        plot(tempx, tempy,'-','linewidth',1.5,'color','red');%%all frames
